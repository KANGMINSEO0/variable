...
//        int result = num1 + num2;   // 자동으로 큰 쪽 자료형인 long으로 변경 후 계산하기 때문에 int형 변수에 값을 담을 수 없다.
        long result = num1 + num2;    // int + long 은 서로 다른 자료형이라 데이터 손실이 발생하지 않는 int -> long 변환을 자동 수행 후 연산한다.
        
        System.out.println("result = " + result);
        
        /* 목차 1-1-2. 상수 끼리의 자동 형변환 */
        float fnum = 4.0f;
        double dnum = fnum;
        ...
        /* 목차 1-3. 문자형은 int형으로 자동 형변환 되다. */
        char ch1 = 'a';                                     // 문자형으로 표현되는 자료형은 2byte의 유니코드 문자로 표현됨
        int charNumber = ch1;

        System.out.println("charNumber = " + charNumber);   // a = 97이기 때문에 97이 담김
